...
box::use(
  ...
  docs/logic/ui[summary_cards, trend_indicator, report_table, modal],
...
```{r js_data}
historico <- historico_bank |>
  bind_rows(historico_infodolar) |>
  dplyr::filter(lubridate::year(date) > 2024) |>
  arrange(date) |> 
  slice(1, .by = c(date, entidad)) |>
  select(date, entidad, buy, sell, gap) |>
  tidyr::pivot_longer(c(buy, sell, gap), names_to = "type", values_to = "tasa") |>
  mutate(
    type = recode(type, "gap" = "Margen", "buy" = "Compra", "sell" = "Venta")
  ) |>
  filter(tasa > 40)

historico_diario <- historico |>
  filter(type != "Margen") |> 
  summarise(tasa = mean(tasa, na.rm = TRUE), .by = c(date, type))

to_js <- historico |> 
  group_by(entidad, type) |>
  summarise(data = list(highcharter::list_parse2(across(c(date, tasa)))), .groups = "drop") |>
  group_by(entidad) |>
  filter(type != "Margen") |> 
  summarise(series = list(
    purrr::map2(type, data, ~ list(name = .x, data = .y))
  ), .groups = "drop") |>
  rename(name = entidad)
...
```{r modal}
modal(
  title = shiny::div(id = "modal-title", "Hello"),
  size = "xl",
  tc_chart(historico_diario, x = "date", y = "tasa", group = "type") |>
    highcharter::hc_chart(
      events = list(
        load = JS("function() { this.myChartId = 'modal-plot'; }")
      )
    )
)
```